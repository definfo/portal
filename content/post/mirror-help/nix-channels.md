...
{ config, lib, pkgs, ... }:
{
  ...
  # 可通过后缀参数 `?priority=n` 指定源优先级 (数值较小优先，1 <= n <= 100), 默认设置下，cache.nixos.org 优先级为 40
  # 默认优先级可在 https://mirror.sjtu.edu.cn/nix-channels/store/nix-cache-info 查看
  # nix.settings.substituters = [ "https://mirror.sjtu.edu.cn/nix-channels/store?priority=10" ];
}
```
